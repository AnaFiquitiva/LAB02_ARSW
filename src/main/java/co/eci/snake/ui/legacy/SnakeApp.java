...
public final class SnakeApp extends JFrame {
    ...
    private void bindKey(Snake s, String key, Direction dir) {
        ...
        gamePanel.getActionMap().put(key, new AbstractAction() {
            @Override
            public void actionPerformed(ActionEvent e) { s.turn(dir); }
        });
    }
    ...
    public static final class GamePanel extends JPanel {
        ...
        protected void paintComponent(Graphics g) {
            ...
            for (var p : board.obstacles()) g2.fillRect(p.x() * cell + 2, p.y() * cell + 2, cell - 4, cell - 4);

            g2.setColor(Color.BLACK); 
            for (var p : board.mice()) g2.fillOval(p.x() * cell + 4, p.y() * cell + 4, cell - 8, cell - 8);

            g2.setColor(Color.RED); 
            for (var p : board.teleports().keySet()) g2.fillOval(p.x() * cell + 6, p.y() * cell + 6, cell - 12, cell - 12);

            g2.setColor(Color.BLUE); 
            for (var p : board.turbo()) g2.fillRect(p.x() * cell + 6, p.y() * cell + 6, cell - 12, cell - 12);
